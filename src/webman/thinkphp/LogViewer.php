...
namespace Wolfcode\PhpLogviewer\webman\thinkphp;
...
class LogViewer extends Base
{
    ...
    public function fetch()
    {
        ...
        $viewBasePath = $this->getPluginBaseViewPath('webman/thinkphp');
        $options      = [
            'view_dir_name' => '',
            'cache_path'    => runtime_path() . DIRECTORY_SEPARATOR . 'views',
            'view_path'     => base_path() . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR
        ];
        ob_start();
        (new Template($options))->fetch($viewBasePath . 'index', static::$vars);
